function [ cleanvars ] = FindCleanDataVariables_NAPLab(datapath,block)
% [ cleanvars ] = FindCleanDataVariables_NAPLab(datapath,block)
%   Runs through various data evaluations to determine:
%       correct audio channel
%       whether to high-pass filter to remove DC offset
%       bad channels
%       best referencing scheme
%       noisy epochs
%       whether to z-score each channel
%
%   Inputs:
%       datapath (optional): subject folder
%           datapath must contain EITHER 
%               data.mat in an 'original' subdirectory 
%               OR
%               .htk files in a 'processed' subdirectory
%       blocks: specify which data block to load for evaluation
%           format: integer
%   Output:
%       cleanvars: data structure with following fields
%           bestaud: integer of best audio channel
%           filters
%                filters.highpass: logical of whether to highpass
%                filters.hishpassfreq: frequency of highpass filter if applied
%           channels
%                channels.bad: indices of clearly noisy channels 
%                channels.iffy: indices of iffy channels
%           reftype: string indicating best reference scheme
%           amplifiers: split of groups if reference is split into groups
%           epochs: sample ranges of bad epochs
%           normdata: logical of whether to z-score data by channel
%
% Code will also ask whether to save cleanvars in processed directory
...
%% Try removing DC with high pass filter
