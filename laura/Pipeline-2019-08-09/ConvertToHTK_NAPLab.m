function ConvertToHTK_NAPLab(datapath, datatype, blocks, channels, dataf)
%   ConvertToHTK_NAPLab(datapath, datatype, blocks, channels, dataf)
...
%           options: 'blackrock' 'edf' 'gtec' 'tdt1/2/3'
...
%       dataf (optional): specify desired data fs
%           default: 1000Hz UNLESS original fs is less than 1000,
%               in which case it is not resampled
%
% Result:
...
% Updated 06/10/2019 by Laura
% Standardizing for inclusion in lab-wide package
%
...
% Data sampling rate defaults to 1000 and audio sampling rate defaults to 24k
if ~exist('dataf','var') || isempty(dataf) % sampling frequency default to 1000
    dataf = 1000;
end
if ~exist('audf','var') || isempty(audf) % sampling frequency default to 24k
    audf = 24000;
end
...
for i = 1:length(blocknames)
    ...
    switch lower(datatype)
        ...
        case 'tdt2+' % Added by Menoua 2019
            
            disp('...loading .tev files');
            
            rawdata = TDTbin2mat([origpath blocknames{i}]);
            
            allfields = fields(rawdata.streams); disp(allfields);
            datafields = input('Which fields are data? ');
            datafields = allfields(datafields);
            %audfields = input('Which fields are audio? ');
            %audfields = allfields(audfields);
            
            %aud = cellfun(@(field) double(rawdata.streams.(field).data), audfields);
            %fs_aud = cellfun(@(field) double(rawdata.streams.(field).fs), audfields);
            %aud = cat(1, aud{:});
            %fs_aud = fs_aud{1};
            
            data = cellfun(@(field) double(rawdata.streams.(field).data), datafields, 'UniformOutput', false);
            fs_data = cellfun(@(field) double(rawdata.streams.(field).fs), datafields, 'UniformOutput', false);
            
            minlen = min(cellfun(@(x) size(x,2), data));
            data = cellfun(@(x) x(:,1:minlen), data, 'UniformOutput', false);
            data = cat(1, data{:});
            fs_data = fs_data{1};
            
            aud = []; fs_aud = [];
            for j = 1:4
                % Extract data from .sev files in that block
                rawdata = SEV2mat([origpath blocknames{i}],'CHANNEL',j,'VERBOSE',0);
                
                aud(j,:) = double(rawdata.Wav5.data); % raw data
                fs_aud(j) = double(rawdata.Wav5.fs); % ECoG fs
            end
            fs_aud = max(fs_aud);
        ...
        case 'edf'
            ...
            % If data.mat is already saved, ask whether to load that instead of original files
            ...
            if loadfromdata
                ...
                try allfs_data = hdr.samples(datach);
                    ...
                    if length(allfs_data) == 1
                        fs_data = allfs_data/hdr.duration;
                    else
                ...
                if trigoraud == 2 || trigoraud == 3
                    ...
                    try allfs_trig = hdr.samples(trigch);
                        ...
                        if length(allfs_trig) == 1
                            fs_trig = allfs_trig/hdr.duration;
                        else
                            error('Multiple fs for data channels');
                        end
                    catch
                ...
                switch trigoraud
                    case 1
                        save([origpath blocknames{i} filesep 'data.mat'],'data','fs_data','chnames','aud','fs_aud','-v7.3');
                        
                    case 2
                        save([origpath blocknames{i} filesep 'data.mat'],'data','fs_data','chnames','trig','fs_trig','-v7.3');
                        
                    case 3
                        save([origpath blocknames{i} filesep 'data.mat'],'data','fs_data','chnames','aud','fs_aud','trig','fs_trig','-v7.3');
        ...
        case 'gtec'
            ...
            warning('Some portions of gtec processing are hard-coded; please check!');
            
            datafile = dir([origpath blocknames{i} filesep '*.hdf5']);
            datafile = datafile.name;
            rawdata = h5read([origpath blocknames{i} filesep datafile], '/RawData/Samples');
            
            data = rawdata(1:63,:); %% CHANNELS ARE HARDCODED
            data = double(data);
            fs_data = 4800; %% HARDCODED
            
            aud = rawdata(65,:); %% CHANNELS ARE HARDCODED
            aud = double(aud);
            fs_aud = 4800; %% HARDCODED
            
            reference = rawdata(64,:); %% CHANNELS ARE HARDCODED
        ...
        case 'blackrock'
            ...
            if exist([origpath blocknames{i} filesep 'data.mat'],'file')
                ...
                for l = 1:length(fnames)
                    ...
                    disp(['Comment: ' file.MetaTags.Comment]); disp('Channels: ');
                    chanlabels = {file.ElectrodesInfo.Label}; numchan = length(chanlabels);
                    for chan = 1:numchan
                        disp([num2str(chan) '      ' chanlabels{chan}]);
                    end
                    audch = input('Which channels are audio? ');
                    trigch = input('Which channels are trigger? ');
                    datach = input('Which channels are data? ');
                    ...
                    if ~isempty(trigch)
                        fs_trig = file.MetaTags.SamplingFreq;
                        thistrig = [];
                        if iscell(file.Data)
                            for ll = 1:length(file.Data)
                                thistrig = cat(2,thistrig,double(file.Data{ll}(audch,:)));
                            end
                        else
                            thistrig = cat(2,thistrig,double(file.Data(audch,:)));
                        end
                        trig = cat(2,trig,thistrig);
                    end
                    if ~isempty(datach)
    ...
    if exist('chnames','var') && ~isempty(chnames)
        ...
        if writechnames
            ...
            WriteChNames_NAPLab(origpath(1:end-10),blocks,trodenames,trodelens);
        end
    end
