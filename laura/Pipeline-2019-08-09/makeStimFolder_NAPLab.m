function StimOrder = makeStimFolder_NAPLab(datapath,blocks,stimpath,task,trials,portion)
% function makeStimFolder_NAPLab
%       []= makeStimFolder_NAPLab(datapath,blocks,stimpath,task,trials)
...
for i = 1:length(blocknames)
    ...
    if fixedorder % if it's a fixed order, read from task file
        ...
        switch lower(task)
            ...
            case 'emotionalspeech' % this needs to be read from resp
            ...
            case 'segmentation' % this may include Presentation logfiles
            ...
            otherwise % default approach is to load directly from .mat in behavioral/B#
