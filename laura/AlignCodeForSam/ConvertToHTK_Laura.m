...
for i = 1:length(blocknames)
    ...
    htkraw_directory = [destpath, blocknames{i}, filesep, 'htkraw'];
    if ~exist(htkraw_directory, 'dir')
        mkdir(htkraw_directory);
    end
    ...
    if exist('aud','var')
        ...
        analog_directory = [destpath, blocknames{i}, filesep, 'analog'];
        if ~exist(analog_directory, 'dir')
            mkdir(analog_directory);
        end
    ...
    if exist('trig','var')
        trigger_directory = [destpath, blocknames{i}, filesep, 'trigger'];
        if ~exist(trigger_directory, 'dir')
            mkdir(trigger_directory);
        end
    ...
    if exist('reference','var')
        ref_directory = [destpath, blocknames{i}, filesep, 'reference'];
        if ~exist(ref_directory, 'dir');
            mkdir(ref_directory);
        end
