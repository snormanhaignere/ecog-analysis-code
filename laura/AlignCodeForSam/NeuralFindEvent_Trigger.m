function evnt = NeuralFindEvent_Trigger(datapath, soundpath, subject, blocks, audchannel, troubleshoot, excludetrigger)
...
for i = 1:length(blocknames)
    ...
    savespecgram = false; %input('Do you want to save the aligned spectrogram? ');
    ...
    %% Load trigger! NEW
    TriggerPath = [datapath filesep 'processed' filesep blocknames{i} filesep 'trigger' filesep 'trigger.htk'];
    [trigger,fs_trig] = readhtk(TriggerPath);
    
    if ~isempty(excludetrigger)
        trigger_inds = find(trigger);
        for j = 1:length(excludetrigger)
            trigger(trigger_inds(excludetrigger(j))) = 0;
        end
    end
    ...
    if exist(StimPath,'file')
        ...
        if savespecgram, loadload; wholeaud = zeros(128, length(audioRecord)); end
    ...
    while soundIndex <= length(StimOrder)  % loop the audio file names
        ...
        if goToNext == 1
            ...
            if savespecgram
                fs_audforspecgram = 16000; % NEED TO CHECK WITH BAHAR ABOUT THIS
                audioforspecgram = resample(w,fs_audforspecgram,fs_audio); 
                audioforspecgram = w; fs_audforspecgram = fs_audio;
                thisaud = wav2aud(audioforspecgram, [1000/fs_trig 1000/fs_trig -2 log2(fs_audforspecgram/16000)] )';
            end
...
%             w2 = resample(w,audioRecordFreq,fs_audio);
        ...
        if exist(StimPath,'file')
            ...
            if savespecgram,
                wholeaud(:,lastSyncPoint:lastSyncPoint+length(thisaud)-1) = thisaud;
            end
    ...
    if exist('wholeaud','var');
        figure; 
        imagesc(wholeaud); axis xy; colormap jet; hold on;
        plot(trigger*50,'w','LineWidth',3);
        title('Spectrogram from File      Trigger: White');
    end
    
    
    % Ask about saving
    saveevnt = 'y'; %input('Save evnt for this block? ','s');
    ...
    savewaveform = 'y'; %input('Save waveform for this block? ','s');
    ...
    if savespecgram
        savespecgram = input('Save spectrogram for this block? ','s');
        if ~strcmpi(savespecgram,{'y', 'yes', '1'})
        else
            loadload;
            aud = wholeaud;
            savename = [datapath filesep 'processed' filesep blocknames{i} filesep 'Stimulus' filesep 'spectrogram.htk'];
            writehtk(savename,aud,fs_trig);
            disp(['Saved to ' savename]);
        end
    end
