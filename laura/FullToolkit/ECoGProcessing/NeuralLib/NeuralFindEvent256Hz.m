...
%             x = [SoundPath filesep StimOrder{soundIndex}];
            ...
            %w=padarray(w,[fs_audio/2 0],'both');
            w2 = resample(w,audioRecordFreq,fs_audio);  %downsample to 2400 Hz 
            goToNext = 0;
        end
        
        % Cross-correlation
        windowSize = ceil(length(w2)/audioRecordFreq)+1.5;
        audioPart = audioRecord(lastSyncPoint+lastLength+1:...
            min(lastSyncPoint+lastLength+windowSize*audioRecordFreq, stop)); % sampling rate is 10K, find the xcorr in the next 20s window
        
        
        w2 = (w2-mean(w2)) / std(w2);
        %w2(abs(w2)<(std(w2)/5)) = 0; 
        audioPart = (audioPart-mean(audioPart))/std(audioPart);
        %audioPart(abs(audioPart)<(std(audioPart)/5)) = 0; 
...
%         tmp = audioRecord(syncPosition(end)+1:min(syncPosition(end)+length(waveform),length(audioRecord)) );
        ...
        % added by Tasha
        waveform(abs(waveform)<(std(waveform)/10)) = 0; 
        recordWaveform(abs(recordWaveform)<(std(recordWaveform)/10)) = 0;
...
%         confidence=(confidence+confidence2)/2;
        if confidence > 0.1
            goToNext = 1;
            lastLength =length(w2);
            %figure; plot(recordWaveform); hold on; plot(waveform,'r'); hold off; title(num2str(confidence))
        else
            disp('Warning: confidence too low.')
            figure; plot(recordWaveform); hold on; plot(waveform,'r'); hold off; title(num2str(confidence))
            break
        end
