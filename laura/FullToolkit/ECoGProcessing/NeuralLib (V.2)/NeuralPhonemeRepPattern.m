function [phoneme_data,phoneme_feat,senames,dur] = ...
    NeuralPhonemeRepPattern(s, out,pattern,includeflag, phoneme, fname, param,probabilityint,average_single, feats,loc,gender)
% s = ECSpeech;
% out = data (output .mat file from DNN)
%pattern= findes the exact phones that are mentioned in the pattern ( in
%the case of includeflag==1) or phones that are not in the pattern in the
%case of includeflag==0
...
temp=struct2cell(pattern);
phons=squeeze(temp(1,1,:));
ind=find(strcmp(phons,phoneme));
phnpattern=pattern(ind);
