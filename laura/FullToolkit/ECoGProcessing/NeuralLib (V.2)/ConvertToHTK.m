...
% LAURA EDIT: Create dest if not already present  
if ~exist(dest,'dir')
    disp('Making dest directory')
    mkdir(dest);
end  % END LAURA EDIT
...
for i=1:length(filenames)
    if ~isempty(strfind(filenames{i},'stimorder'))
        ...
        part=str2double(splitted(2));
        if ~isempty(strfind(filenames{i},'stimorderA'))
            copyfile([org,'original/',filenames{i}],[dest,'B',int2str(part),'/Stimulus/StimOrderA.mat']);
        elseif ~isempty(strfind(filenames{i},'stimorderB'))
            copyfile([org,'original/',filenames{i}],[dest,'B',int2str(part),'/Stimulus/StimOrderB.mat']);
        else
