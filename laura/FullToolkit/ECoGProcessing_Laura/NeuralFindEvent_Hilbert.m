function evnt = NeuralFindEvent_Hilbert (datapath, soundpath, subject, blocks, audchannel, troubleshoot, needhilb, confidencethreshold, checkmanually)
...
if ~exist('needhilb','var') || isempty(needhilb)
    needhilb = [];
end
if ~exist('confidencethreshold','var') || isempty(confidencethreshold)
    confidencethreshold = 0.6;
end
if ~exist('checkmanually','var') || isempty(checkmanually)
    checkmanually = [];
end
...
for i = 1:length(blocknames)
    ...
    % Set up errorlogical for hilbert-needed stimuli
    errorlogical = false(1,length(StimOrder));
    if iscell(needhilb)
        errorlogical(needhilb{i}) = true;
    else
        errorlogical(needhilb) = true;
    end
    manualchecklogical = false(1,length(StimOrder));
    if iscell(checkmanually)
        manualchecklogical(checkmanually{i}) = true;
    else
        manualchecklogical(checkmanually) = true;
    end
    ...
    while soundIndex <= length(StimOrder)  % loop the audio file names
        ...
        if confidence > confidencethreshold % Currently set to .6; can discuss to determine ideal threshold.
