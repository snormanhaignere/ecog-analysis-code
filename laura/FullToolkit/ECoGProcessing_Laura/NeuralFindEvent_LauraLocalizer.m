function evnt = NeuralFindEvent_Hilbert (datapath, soundpath, subject, task, blocks, audchannel, troubleshoot)
...
for i = 1:length(blocknames)
    ...
    switch task
        case 'Localizer'
            switch subject
                case '025_LIJ109'
                    needhilb = [1 6 11 14 31 38 48 58];
                    errorlogical(needhilb) = true;
                case '026_LIJ110'
                    needhilb = [1 6 11 14 32 39 49 59 67];
                    errorlogical(needhilb) = true;
            end
        case 'NoiseAdaptation'
            needhilb = true(1,length(StimOrder));
    end
    errorlogical(needhilb) = true;
...
%             w2 = resample(w,audioRecordFreq,fs_audio);
        ...
        if confidence > 0.5 % Currently set to .6; can discuss to determine ideal threshold.
