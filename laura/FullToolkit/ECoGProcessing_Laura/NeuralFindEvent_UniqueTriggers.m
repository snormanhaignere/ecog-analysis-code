function evnt = NeuralFindEvent_UniqueTriggers (datapath, soundpath, subject, blocks, audchannel, troubleshoot, needhilb, confidencethreshold, checkmanually)
...
for i = 1:length(blocknames)
    ...
    if length(StimOrder) ~= sum(trigger)
        error('StimOrder length does not match number of triggers');
    end
