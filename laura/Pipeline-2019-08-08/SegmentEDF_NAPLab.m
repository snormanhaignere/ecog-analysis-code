...
pth = ['/Users/svnh2/Desktop/projects/speech-TCI/data/ECoG-Raw/' subject filesep]; % Path to the location you have set up the proper file structure
block = 1; % which block you are currently processing; run this script once per block
...
datachan = 1:174; % Enter range of data channels
trigch = 257; % Enter trigger channel index
audch = 258; % Enter audio channel index (if no audio is available, I double-save trigger as audio)
...
switch block
    case 1
        blockrng = 510449:2063805; % SAMPLE RANGE OF BLOCK 1 HERE
    case 2
...
figure; plot(data_1(audch,blockrng));
...
trig = alldata(trigch,:); fs_trig = hdr.samples(trigch(1))/hdr.duration; trignames = hdr.label(trigch);
aud = alldata(audch,:); fs_aud = hdr.samples(audch(1))/hdr.duration; audnames = hdr.label(audch);
orighdrs = hdr;
